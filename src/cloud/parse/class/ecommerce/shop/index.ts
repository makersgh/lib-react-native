...
import { ICoordinates, Staff } from '..';
import { ShopDay } from './ShopDay';
import { getDay, phoneNumberValidator } from 'lib_helpers';
...
export class Shop extends ParseBaseClass {
  ...
  getSectionedMenu() {
    ...
    const data = this.menu?.reduce((accumulator, currentValue) => {
      const sectionalEntry = accumulator.find((item) => item.title === currentValue.category);
      if (!sectionalEntry) {
        accumulator.push({
    ...
    try {
      if (this.opened) {
        ...
        if (!this.image) {
          ...
          const areDaysAvailable = Object.values(this.availability).some((entry) => entry.opened);
          if (!areDaysAvailable) {
      ...
      if (errortext) {
        return Promise.reject('Your shop cannot be opened at this time. ' + errortext);
      } else {
        this.opened = true;
        await this.save();
